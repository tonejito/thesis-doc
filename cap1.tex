...
Este principio busca conservar los datos \'{u}nicamente para la persona que est\'{a} destinada a leerlos \cite{NIST} \cite{oregon-sec}.
...
Este principio busca que la informaci\'{o}n no pueda ser alterada, ya sea por fallos en el medio de almacenamiento o modificaciones no autorizadas \cite{NIST} \cite{oregon-sec}.
...
Este principio dicta que la informaci\'{o}n debe poderse acceder cuando sea necesario o respetando el criterio de los tiempos establecidos \cite{NIST} \cite{oregon-sec}.
...
Este protocolo se utiliza para copiar archivos entre dos equipos, ya sea desde el equipo remoto hacia el local o viceversa y utiliza \textsc{SSH} como transporte para los datos. Est\'{a} basado en el programa de copia de archivos \textit{rcp} originario de la variante de \textsc{UNIX} \textsc{BSD} \cite{FreeBSD-scp}.

  \subsection {SFTP - Secure FTP}

El protocolo \textsc{SFTP} dise\~{n}ado por la \textsc{IETF} \footnote{Espec\'{i}ficamente por el grupo \textsc{SECSH} de la \textsc{IETF}.} es una extensi\'{o}n del protocolo \textsc{SSH} que a diferencia de \textsc{SCP} permite acceder, transferir y borrar archivos adem\'{a}s de listar los directorios del equipo remoto \cite{ipsure}. La versi\'{o}n actual de este protocolo es la 6.0. 

  \subsection {SSHFS - Secure Shell Filesystem}

\textsc{SSHFS} es un programa que hace uso de \textsc{SFTP} para \textit{montar} un directorio remoto en el equipo local utilizando \textsc{SSH} como transporte \cite{ubuntu-sshfs} y hace uso de la biblioteca de sistemas de archivo \textsc{FUSE} \cite{FUSE} del lado del cliente para realizar el mapeo y manejo del sistema de archivos virtual. El funcionamiento de un directorio montado con \textsc{SSHFS} es similar a \textsc{NFS} o \textsc{CIFS} \cite{linuxjournal}.
