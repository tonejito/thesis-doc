...
El sistema operativo GNU/Linux es la combinaci\'{o}n del conjunto de utilerias del sistema operativo GNU \footnote{Proyecto iniciado en 1983 por Richard Stallman.} y el kernel Linux \footnote{Desarrollado por Linus Torvalds y liberado por primera vez en 1991.} \cite{_linux_????}.

  \subsection {Historia}

En 1983 Richard Stallman inici\'{o} el proyecto \textsc{GNU} \footnote{Acr\'{o}nimo recursivo que significa \textsc{GNU} is Not Unix.} cuyo prop\'{o}sito es hacer un sistema operativo compatible con \textsc{UNIX} que cumpla con las cuatro libertades del software \footnote{Para m\'{a}s informaci\'{o}n ver el ap\'{e}ndice A.} y desarroll\'{o} junto con otros colegas una serie de utilerias y programas como el compilador \textsc{GNU} para el lenguage \textsc{C} (\textit{gcc}) \cite{_about_????}. Sin embargo el proyecto ten\'{i}a pr\'{a}cticamente todos los programas del sistema operativo pero le faltaba la parte escencial, el kernel.

En 1987 el profesor Andrew Tannenbaum public\'{o} un libro titulado \textit{Sistemas Operativos Dise\~{n}o e Implementaci\'{o}n} \cite{tanenbaum_operating_2006} donde al final del mismo viene el c\'{o}digo fuente del sistema operativo \textsc{MINIX} que el mismo desarroll\'{o} como un clon de \textsc{UNIX} para prop\'{o}sitos educativos. \cite{_complete_????}.

En 1990 Linus Torvalds que estudiaba la maestría en ciencias de la computación en Helsinki Finlandia 
