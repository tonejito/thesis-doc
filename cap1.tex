...
Est\'{a} definido en el RFC 4918 \cite{rfc4918} y establece un conjunto de extensiones al protocolo HTTP que incluyen m\'{e}todos, cabeceras y formatos tanto de petici\'{o}n como de respuesta para interactuar con el servidor con la finalidad de crear, modificar, borrar archivos y adem\'{a}s establece un mecanismo denominado \textit{File locking} que previene que dos personas editen el documento al mismo tiempo para no perder cambios.

\section {Protocolo LDAP}

El protocolo LDAP \footnote{Ligthweight Directory Access Protocol por sus siglas en ingl\'{e}s.} est\'{a} definido en el RFC 4511 \cite{rfc4511} y establece un directorio como una colecci\'{o}n de objetos que comparte a otros equipos y programas a trav\'{e}s de una conexi\'{o}n de red.
...
La nomenclatura de los objetos es similar a la de DNS, es decir el nivel m\'{a}s general se indica a la derecha y el nivel m\'{a}s espec\'{i}fico se indica a la izquierda.
{
\scriptsize
\linespread{1}
\begin{verbatim}
