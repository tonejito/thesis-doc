...
Un \textit{appliance} es un conjunto de elementos (\emph{\gls{hardware}}, \emph{\gls{software}} y sistema operativo) que trabajan de manera conjunta para realizar un proceso espec\'{i}fico \cite{smith_linux_2007}.
...
Tras el descubrimiento de vulnerabilidades en el algoritmo \textsc{DES}, en 1998 la \textsc{\gls{EFF}}\footnote{\emph{Electronic Frontier Foundation} por sus siglas en ingl\'{e}s.} public\'{o} en Internet la especificaci\'{o}n de una m\'{a}quina capaz de romper el cifrado de este algoritmo\cite{_criptografi-_????}\cite{<scotthyperthought.com>_security_????}. La soluci\'{o}n interina que fue adoptada consiste en realizar el cifrado \textsc{DES} \emph{tres veces}, es decir, cifrar el mensaje con \emph{Triple-DES}.
...
A continuaci\'{o}n se describen algunas funciones \emph{hash} empleadas en \textsc{\gls{SSL}} y \textsc{\gls{TLS}}:
...
% El algoritmo \textsc{RC4} no ser\'{a} soportado en \textsc{\gls{TLS}} puesto que el \textsc{\gls{RFC}}7465 lo prohibe expresamente debido a que se han encontrado fallas explotables en el\cite{<andreipomicrosoft.com>_prohibiting_????}. %rfc7465

        \item \textbf {MD5}

Fue creado por Ron Rivest de \textsc{RSA} y publicado en el \textsc{\gls{RFC}}1321\cite{rivest_md5_????}, procesa la entrada en bloques de 512 bits y obtiene como salida una firma con longitud de 128 bits.

Debido a que se han encontrado \emph{colisiones}\footnote{Dos mensajes diferentes que generan el mismo valor de \emph{hash}.} en el algoritmo \textsc{MD5} es considerado \textbf{no seguro}\cite{project_weak_2013} y se ha prohibido su uso junto con \textsc{SSLv2} en el \textsc{\gls{RFC}}6176\cite{<>_prohibiting_????}.

        \item \textbf {SHA1 y SHA256}

Publicada por el \textsc{NIST} en el \textsc{\gls{RFC}}3174\cite{3rd_us_????}, \textsc{SHA1} obtiene una salida de 160 bits al procesar de manera secuencial bloques de entrada con longitud de 512 bits.

\textsc{SHA256} proviene de la familia \textsc{SHA-2} publicada en el \textsc{\gls{RFC}}4634\cite{hansen_us_????}, produce una salida de longitud fija con 256 bits y procesa bloques de 512 bits de longitud. A\'{u}n no se han encontrado vulnerabilidades o colisiones en esta familia de funciones, por lo que se consideran seguras.
...

{
 ...
 {%
  ...
  \hline \hline
  \textit{Domain Component} & \texttt{dc=tonejito,dc=org} \\
  \textit{Organization}     & \texttt{o=tonejito} \\
  %\hline
  \end{tabular}
 } % ending of \makebox
 \end{table}
...
%
%Este protocolo se utiliza para copiar archivos entre dos equipos, ya sea desde el equipo remoto hacia el local o viceversa y utiliza \textsc{\gls{SSH}} como transporte para los datos. Est\'{a} basado en el programa de copia de archivos \textit{rcp} originario de la variante de \textsc{UNIX} \textsc{BSD} \cite{_scp_????}.
%
%  \subsection {SFTP - Secure FTP}
%
%El protocolo \textsc{SFTP} dise\~{n}ado por la \textsc{IETF} \footnote{Espec\'{i}ficamente por el grupo \textsc{SECSH} de la \textsc{IETF}.} es una extensi\'{o}n del protocolo \textsc{\gls{SSH}} que a diferencia de \textsc{SCP} permite acceder, transferir y borrar archivos adem\'{a}s de listar los directorios del equipo remoto \cite{_chrooted_????}. La versi\'{o}n actual de este protocolo es la 6.0.
%
%  \subsection {SSHFS - Secure Shell Filesystem}
%
%\textsc{SSHFS} es un programa que hace uso de \textsc{SFTP} para \textit{montar} un directorio remoto en el equipo local utilizando \textsc{\gls{SSH}} como transporte \cite{_sshfs_????} y hace uso de la biblioteca de sistemas de archivo \textsc{FUSE} \cite{_ssh_????} del lado del cliente para realizar el mapeo y manejo del sistema de archivos virtual. El funcionamiento de un directorio montado con \textsc{SSHFS} es similar a \textsc{NFS} o \textsc{CIFS} \cite{_sshfs:_????}.
%
