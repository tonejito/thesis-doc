...

{
 ...
 {%
  ...
  \hline \hline
  \textit{Protocolo} & \texttt{Contenedor} \\
  \textit{Host}      & \texttt{Atributos de \'{i}ndice} \\
  \textit{Puerto}    & \textsc{Profundidad y condiciones de la b\'{u}squeda} \\
  %\hline
  \end{tabular}
 } % ending of \makebox
 \end{table}
...
El programa utiliza de manera interna el comando \texttt{net use} \footnote{https://technet.microsoft.com/en-us/library/gg651155.aspx} que permite montar una carpeta del servidor \textsc{WebDAV} con una letra de unidad local (Ej. \texttt{Z:}). Es una alternativa a la interfaz est\'{a}ndar que presenta el \textit{Explorador de Windows} \footnote{http://windows.microsoft.com/en-us/windows/create-shortcut-map-network-drive}
